...
class MainWindow(AcrylicWindow, Ui_Form):
    ...
    def run(self):
        ...
        cmd = f'start cmd /c "scrcpy.exe {" ".join(args)} || echo. & echo.\033[31m\033[1m---Scrcpy进程已结束 按任意键关闭窗口---\033[0m & pause>nul"'
    ...
    def restart_adb(self):
        ...
        def _():
            subprocess.run('adb kill-server', shell=True, creationflags=subprocess.CREATE_NEW_CONSOLE)
            subprocess.run('adb start-server', shell=True, creationflags=subprocess.CREATE_NEW_CONSOLE)
